...
if (iconMenu != null) {
	let delay = 500;
	let menuBody = document.querySelector(".header__wrapper");
...
"use strict"
// =====================input tel====================
let inp = document.getElementById("phone-mask");
...
validation
	.addField('.input-tel', [
		...
		{
			rule: 'customRegexp',
			value: /^\+380\d{2}\d{3}\d{4}$|^\+380 \d{2} \d{3} \d{4}$/,
			errorMessage: '*Номер повинен починатися з +380',
		},
	]).onSuccess((event) => {
...
// =====================text circle====================
const text = document.querySelector(".star-map__text pre");
...
var scene = document.getElementById('scene-4');
var parallaxInstance = new Parallax(scene, {
	hoverOnly: true
});
